...
\title[Exercises 3 $\cdot$ SDS 383D]{Exercises 3: Linear smoothing and Gaussian processes}
%\author[ ]{ }
...
\begin{enumerate}[(A)]
\item   Read up on the \href{https://en.wikipedia.org/wiki/Matérn_covariance_function}{Matern class}\footnote{\url{https://en.wikipedia.org/wiki/Matérn_covariance_function}} of covariance functions.  The Matern class has the \textit{squared exponential} covariance function as a special case:
$$
...
$$
where $d = \Vert x_1 - x_2 \Vert_2$ is the distance between the two points $x_1$ and $x_2$.  Comment on the differences between the functions generated from the two covariance kernels.\footnote{The Mat\'ern covariance is actually a whole family of functions: \url{http://en.wikipedia.org/wiki/Matérn_covariance_function}.}
