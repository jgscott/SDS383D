...
$$
\hat{\beta} = \arg \min_{\beta \in \mathcal{R}^p} \Vert (y - X\beta) \Vert_2^2 = (y - X\beta)^T (y-X \beta)
$$
...
$$
Here $p_i(y_i \mid \sigma^2)$ is the conditional probability density function of $y_i$, given the model parameters $\beta$ and $\sigma^2$.  Note that an equivalent way to write this is to say that $y$ is multivariate normal with mean $X \beta$ and covariance matrix $\sigma^2 I$, where $I$ is the $n$-dimensional identity matrix.  
